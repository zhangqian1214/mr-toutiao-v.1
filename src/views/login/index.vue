    ...
    <el-card class="my-card">
      ...
      <el-form ref="loginForm" :model="loginForm" :rules="loginRules" status-icon>
        ...
        <el-form-item prop="code">
          <el-input v-model="loginForm.code" placeholder="请输验证码" style="width:232px;margin-right:10px"></el-input>
...
export default {
  data () {
    ...
    const checkMobile = (rule, value, callback) => {
      ...
      if (!/^1[3-9]\d{9}$/.test(value)) return callback(new Error('手机格式不正确'))// 原来可以new错误对象
      callback()// 成功就跳过if，返回空函数，表示校验成功
    }
    return {
      loginForm: {
        // 数据函数返回一个表单数据对象,跟表单双向绑定，发回服务器
        mobile: '', // 字段参考接口文档（后端给的）
        code: ''
      },
      // 具体校验规则查阅element-UI文档
      loginRules: {// 校验规则对象,对用户输入的信息进行校验，通过才发送服务器
        mobile: [ // 校验规则可能有多套，所以用数组
  ...
  methods: {
    ...
    // 要操作dom，记得给表单加ref属性；validate是校验方法，里面是一个回调函数，参数valid是判断校验是否成功，查E-ui文档来的
      this.$refs.loginForm.validate((valid) => { // 校验成功与否就看传回valid的值，不用管，是E-ui自动给的？
        if (valid) { // 如果校验成功,就发ajax，请求类型、地址、都查文档；$http是自定义属性名，是vue上挂载的axios方法
          this.$http.post('http://ttapi.research.itcast.cn/mp/v1_0/authorizations', this.loginForm)// this.loginForm是要发给服务器的对象形式数据
            .then(res => { // res是响应对象，里面有响应行、响应头、响应体
              // console.log(res.data)// res.data是响应体（服务器返回的数据），验证成功测试
              this.$router.push('/')// 登录成功追加跳转到主页面
            })
            .catch(() => { // catch是什么？是axios提供的请求错误回调
              // 请求失败提示
